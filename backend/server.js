...
app.post('/api/exam/pdf-upload', pdfUpload.single('pdf'), async (req, res) => {
  ...
  try {
    ...
    if (!req.body.title || !req.body.subject || !req.body.testNumber) return res.status(400).json({ error: "title, subject, testNumber required" });
...
function parseQuestionsFromText(rawText) {
  ...
  const kannadaOptionRegex = /^[ಎಬಿಸಿಡಿ]\)/;
...
// PDF DRAFT ROUTES (unchanged)
app.get('/api/pdf-drafts', async (req, res) => {
  res.json(await PdfQuestionDraft.find().sort({ createdAt: -1 }));
});
...
app.patch('/api/pdf-draft/:id/set-answer', async (req, res) => {
  ...
  if (questionIndex == null || !correctAnswer) return res.status(400).json({ error: "questionIndex and correctAnswer required" });
  try {
    const draft = await PdfQuestionDraft.findById(req.params.id);
    if (!draft) return res.status(404).json({ error: "Draft not found" });
...
app.post('/api/pdf-draft/:id/finalize', async (req, res) => {
  try {
    ...
    if (missing) return res.status(400).json({ error: "Missing answers" });
