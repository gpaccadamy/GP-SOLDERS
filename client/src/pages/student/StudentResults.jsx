...
export default function StudentResults() {
  ...
  const [screen, setScreen] = useState(token ? "results" : "login");
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (res.ok && data.token) {
        ...
        if (!data.active) {
          setLoginError("Your account is inactive. Contact administrator.");
          return;
        }
        localStorage.setItem("gp_token", data.token);
        localStorage.setItem("gp_name", data.name || "Soldier");
        setToken(data.token);
        setStudentName(data.name || "Soldier");
        setScreen("results");
  ...
  useEffect(() => {
    if (!token || screen !== "results") return;
      ...
      {screen === "login" ? (
        ...
        <div style={styles.loginContainer}>
          <div style={styles.loginCard}>
            <h1 style={styles.loginTitle}>My Results</h1>
            <p style={styles.loginSubtitle}>Login to view your exam results</p>
            ...
            <button onClick={() => navigate("/student")} style={styles.backLink}>Back to Home</button>
          </div>
        </div>
      ) : (
        // RESULTS SCREEN
...
const styles = {
  ...
  // Login Styles
  loginContainer: { minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg, #1e4d6b 0%, #2d6a4f 100%)" },
  loginCard: { background: "#1e2937", borderRadius: "16px", padding: "40px", width: "100%", maxWidth: "400px", boxShadow: "0 10px 40px rgba(0,0,0,0.3)" },
  loginTitle: { margin: "0 0 8px 0", fontSize: "1.8rem", fontWeight: "700", color: "#e2e8f0", textAlign: "center" },
  loginSubtitle: { margin: "0 0 28px 0", fontSize: "0.95rem", color: "#94a3b8", textAlign: "center" },
  loginForm: { marginBottom: "20px" },
  inputGroup: { position: "relative", marginBottom: "16px", display: "flex", alignItems: "center" },
  inputIcon: { position: "absolute", left: "12px", color: "#60a5fa" },
  input: { width: "100%", padding: "12px 40px", background: "#0f172a", border: "1px solid #334155", borderRadius: "8px", color: "#e2e8f0", fontSize: "0.95rem", outline: "none", boxSizing: "border-box" },
  errorMsg: { color: "#ef4444", fontSize: "0.85rem", margin: "0 0 12px 0", textAlign: "center" },
  loginBtn: { width: "100%", padding: "12px", background: "#3b82f6", color: "#fff", border: "none", borderRadius: "8px", fontSize: "1rem", fontWeight: "600", cursor: "pointer", marginBottom: "12px" },
  backLink: { width: "100%", padding: "10px", background: "none", color: "#60a5fa", border: "1px solid #60a5fa", borderRadius: "8px", fontSize: "0.95rem", cursor: "pointer" }
};